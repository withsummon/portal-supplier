...
test.describe('Portal smoke', () => {
  ...
  test('admin routes render without legacy partner surface', async ({ page }) => {
    ...
    await expect(page.locator('a[href^="/admin/products/"] img').first()).toBeVisible()
    await page.getByRole('button', { name: /table/i }).click()
    await page.getByRole('link', { name: /add item/i }).click()
    await expect(page).toHaveURL(/\/admin\/products\/new$/i, { timeout: 10000 })
    await expect(page.getByRole('heading', { name: /add factory item/i })).toBeVisible()
    await expect(page.locator('[role="dialog"]')).toHaveCount(0)
    await page.goto('/admin/products')
    const adminProductLink = page.locator('tbody a[href^="/admin/products/"]').first()
    await expect(adminProductLink).toBeVisible({ timeout: 10000 })
    await adminProductLink.click()
    await expect(page).toHaveURL(/\/admin\/products\/[^/]+$/i, { timeout: 10000 })
    await expect(page.getByRole('link', { name: /back to catalog/i })).toBeVisible()
    await expect(page.locator('[role="dialog"]')).toHaveCount(0)
    await page.getByRole('link', { name: /edit item/i }).click()
    await expect(page).toHaveURL(/\/admin\/products\/[^/]+\/edit$/i, { timeout: 10000 })
    await expect(page.getByRole('heading', { name: /edit factory item/i })).toBeVisible()
    await expect(page.locator('[role="dialog"]')).toHaveCount(0)
    await expectHealthyPage(page, '/admin/articles', /articles/i)
    await page.getByRole('link', { name: /add article/i }).click()
    await expect(page).toHaveURL(/\/admin\/articles\/new$/i, { timeout: 10000 })
    await expect(page.getByRole('heading', { name: /add article/i })).toBeVisible()
    await expect(page.locator('[role="dialog"]')).toHaveCount(0)

...
import { db } from "@/db";
import { projects, sellers, quotes, vendors } from "@/db/schema";
...
export const getCachedCategoryData = React.cache(async () => {
  const projectList = await db
    .select({ category: projects.category, budgetMin: projects.budgetMin, budgetMax: projects.budgetMax })
    .from(projects);
...
export const getCachedPipelineData = React.cache(async () => {
  const projectList = await db
    .select({ status: projects.status, budgetMin: projects.budgetMin, budgetMax: projects.budgetMax })
    .from(projects);
  ...
  return {
    accepted: { label: "Accepted", value: revenueMap["ACCEPTED"] ?? 0, color: "var(--color-success)" },
    inProgress: { label: "In Progress", value: revenueMap["IN_PROGRESS"] ?? 0, color: "var(--blue-500)" },

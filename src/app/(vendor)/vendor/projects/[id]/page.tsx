...
function getBudgetValue(budgetRange: string): { amount: string; currency: string } {
    const match = budgetRange.match(/\$?([\d,]+)K?/g);
    if (match && match.length >= 1) {
        const last = match[match.length - 1].replace(/[,$]/g, '');
        if (budgetRange.includes('K')) {
            return { amount: last, currency: 'K USD' };
        }
        return { amount: last, currency: 'USD' };
    }
    ...
    if (!project) {
        return (
            <div style={{ padding: 'var(--sp-20)', textAlign: 'center' }}>
                <Briefcase size={48} style={{ color: 'var(--text-muted)', marginBottom: 'var(--sp-4)' }} />
                <h2 style={{ fontSize: 'var(--fs-xl)', fontWeight: 'var(--fw-bold)', marginBottom: 'var(--sp-2)' }}>Project not found</h2>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)' }}>
                                    {project.deliverables.map((d, i) => (
                                        <div key={i} style={{
                                            ...
                                            <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-primary)', lineHeight: 1.6 }}>{d}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="drawer-body">
                        <form id="bid-form" onSubmit={handleSubmitBid} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-6)' }}>
                            <div className="form-group">
                                ...
                                <span className="form-hint">Expected budget: {project.budgetRange}</span>
                            </div>

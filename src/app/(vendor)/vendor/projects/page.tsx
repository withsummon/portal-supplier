...
import { mockProjects, statusLabels, formatDate } from '@/lib/mock-data';
...
function getBudgetValue(budgetRange: string): string {
    // Extract the upper bound or the single value
    const match = budgetRange.match(/\$?([\d,]+)K?/g);
    if (match) {
        const last = match[match.length - 1].replace(/[,$K]/g, '');
        return last;
    }
    return '0';
}
...
export default function ProjectMarketplace() {
    ...
    const openProjects = mockProjects.filter(p =>
        (p.status === 'accepted' || p.status === 'submitted' || p.status === 'under_review' || p.status === 'need_clarification') &&
        (activeCategory === 'All' || p.category === activeCategory) &&
        (activeStatus === null || p.status === activeStatus) &&

...
import { mockQuotes, quoteStatusLabels, formatDate } from '@/lib/mock-data';
...
export default function MyQuotesPage() {
    ...
    const stats = [
        { label: 'Total Bids', value: mockQuotes.length, icon: Clock, color: 'var(--blue-600)' },
        { label: 'Pending', value: mockQuotes.filter(q => q.status === 'pending').length, icon: AlertCircle, color: 'var(--color-warning)' },
        { label: 'Won', value: mockQuotes.filter(q => q.status === 'accepted').length, icon: CheckCircle, color: 'var(--color-success)' },
        { label: 'Rejected', value: mockQuotes.filter(q => q.status === 'rejected').length, icon: XCircle, color: 'var(--color-danger)' },
    ];
        ...
        <div className="animate-in">
            ...
            <div className="card">
                <div className="card-header" style={{ padding: 'var(--sp-4) var(--sp-6)' }}>
                    <div className="header-search" style={{ border: 'none', background: 'var(--neutral-50)', width: '100%', maxWidth: '360px' }}>
                        <Search size={16} />
                        <input 
                            type="text" 
                            ...
                            {filteredQuotes.map((quote) => (
                                <tr key={quote.id}>
                                    <td>
                                        ...
                                        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{quote.id}</div>
                                    </td>
                                    <td>

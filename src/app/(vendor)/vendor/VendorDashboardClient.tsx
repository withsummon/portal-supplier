            ...
            {availableProjects.map((project) => (
              <Link href={`/vendor/projects/${project.id}`} key={project.id} style={{ textDecoration: "none", color: "inherit" }}>
                <div style={{
                  ...
                  transition: "all 200ms ease", cursor: "pointer", background: "white",
                }}>
                  <div style={{
                    width: "80px", height: "80px", borderRadius: "var(--radius-lg)",
                    background: getBannerGradient(project.category), display: "flex",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "var(--sp-1)" }}>
                      <h3 style={{ fontSize: "var(--fs-sm)", fontWeight: "var(--fw-bold)", color: "var(--text-primary)", lineHeight: 1.3 }}>
                        {project.name}
                      </h3>
                      <StatusBadge status={project.status as "accepted" | "submitted"} />
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-2)", marginBottom: "var(--sp-2)" }}>
                      <span style={{ padding: "2px 8px", background: `${CATEGORY_COLORS[project.category] ?? "#2563eb"}15`, color: CATEGORY_COLORS[project.category] ?? "#2563eb", borderRadius: "var(--radius-sm)", fontSize: "10px", fontWeight: "var(--fw-semibold)" }}>
                        {project.category}
                      </span>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", gap: "var(--sp-3)", fontSize: "var(--fs-xs)", color: "var(--text-muted)" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: "4px" }}><DollarSign size={12} /> {project.budgetRange ?? "TBD"}</span>
                        <span style={{ display: "flex", alignItems: "center", gap: "4px" }}><Calendar size={12} /> {formatDate(project.startDate)}</span>
                      </div>
                      <span style={{ display: "flex", alignItems: "center", gap: "4px", color: "var(--blue-600)", fontSize: "var(--fs-xs)", fontWeight: "var(--fw-semibold)" }}>
              ...
              <tbody>
                {recentQuotes.map((quote) => (
                  <tr key={quote.id}>
                    ...
                    <td>
                      <StatusBadge status={quote.status as "pending" | "accepted" | "rejected"} type="quote" />
                    </td>
                  </tr>
                ))}
              </tbody>

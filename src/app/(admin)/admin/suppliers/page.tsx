...
import { Search, Filter, MoreVertical, CheckCircle, XCircle, Eye, Mail, Phone, Shield, Star } from 'lucide-react';
...
export default function SuppliersPage() {
    ...
    const getTierColor = (tier: string) => {
        switch (tier) {
            ...
            default:
                return { bg: 'var(--neutral-100)', color: 'var(--text-muted)', border: 'var(--neutral-300)' };
        }
    };
        ...
        <div className="animate-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--sp-4)', marginBottom: 'var(--sp-6)' }}>
                <div className="card" style={{ padding: 'var(--sp-4)' }}>
                    <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-bold)' }}>{suppliers.length}</div>
                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Total Suppliers</div>
                </div>
                <div className="card" style={{ padding: 'var(--sp-4)' }}>
                    <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-bold)', color: 'var(--color-success)' }}>
                        {suppliers.filter(s => s.status === 'active').length}
                    </div>
                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Active</div>
                </div>
                <div className="card" style={{ padding: 'var(--sp-4)' }}>
                    <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-bold)', color: 'var(--color-warning)' }}>
                        {suppliers.filter(s => s.status === 'pending').length}
                    </div>
                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Pending</div>
                </div>
                <div className="card" style={{ padding: 'var(--sp-4)' }}>
                    <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-bold)', color: 'var(--text-muted)' }}>
                        ${suppliers.reduce((acc, s) => acc + parseInt(s.revenue.replace(/[^0-9]/g, '')), 0).toLocaleString()}
                    </div>
                                    ...
                                    <tr key={supplier.id}>
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)' }}>
                                                ...
                                                <div>
                                                    <div style={{ fontWeight: 'var(--fw-semibold)', fontSize: 'var(--fs-sm)' }}>{supplier.name}</div>
                                                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{supplier.email}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{supplier.location}</td>
                                        <td style={{ fontSize: 'var(--fs-sm)' }}>{supplier.projectsCompleted}</td>
                                        <td style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-semibold)' }}>{supplier.revenue}</td>
                                        <td style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-muted)' }}>{supplier.joinedAt}</td>
                                        <td>

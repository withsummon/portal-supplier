...
import { Search, CheckCircle, XCircle, Clock, AlertCircle, Eye, X, MessageSquare, Send, FileText, Calendar, DollarSign, User } from 'lucide-react';
...
const MOCK_PROJECTS: Project[] = [
    { 
        ...
        notes: [{ text: 'Project approved for Q2 execution', by: 'Admin', at: '2024-03-16', type: 'status_change' }]
    },
    { 
        id: 'PRJ-002', 
        ...
        endDate: '2024-08-30',
        priority: 'medium',
        files: [{ name: 'proposal.pdf', size: '1.8 MB' }, { name: 'requirements.docx', size: '500 KB' }],
        notes: []
    },
    { 
        id: 'PRJ-003', 
        ...
        notes: [{ text: 'Please provide more details about your existing data sources and expected daily data volume.', by: 'Admin', at: '2024-03-21', type: 'clarification' }]
    },
    { 
        id: 'PRJ-004', 
        ...
        endDate: '2024-10-15',
        priority: 'critical',
        files: [{ name: 'technical-spec.pdf', size: '5.1 MB' }, { name: 'demo-video.mp4', size: '45 MB' }],
        notes: []
    },
    { 
        id: 'PRJ-005', 
        ...
        priority: 'medium',
        files: [],
        notes: [{ text: 'Budget is below minimum threshold. Please increase budget or contact local consultants.', by: 'Admin', at: '2024-03-11', type: 'status_change' }]
    },
];

const STATUS_CONFIG = {
    submitted: { label: 'Submitted', color: 'var(--blue-600)', bg: 'var(--blue-50)', icon: Clock },
    under_review: { label: 'Under Review', color: 'var(--color-purple)', bg: 'var(--color-purple-bg)', icon: Clock },
    accepted: { label: 'Accepted', color: 'var(--color-success)', bg: 'var(--color-success-bg)', icon: CheckCircle },
    rejected: { label: 'Rejected', color: 'var(--color-danger)', bg: 'var(--color-danger-bg)', icon: XCircle },
    need_clarification: { label: 'Need Clarification', color: 'var(--color-warning)', bg: 'var(--color-warning-bg)', icon: AlertCircle },
};
...
export default function AdminProjectsPage() {
    ...
    const [actionModal, setActionModal] = useState<{ project: Project; action: 'accept' | 'reject' | 'clarify' } | null>(null);
    ...
    const submitAction = () => {
        ...
        const newStatus = action === 'accept' ? 'accepted' : action === 'reject' ? 'rejected' : 'need_clarification';
        ...
        setProjects(prev => prev.map(p => 
            p.id === project.id 
                ? { ...p, status: newStatus, notes: [...p.notes, note] }
                : p
        ));
    ...
    return (
        <div className="animate-in">
            ...
            {/* Status Cards */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 'var(--sp-4)', marginBottom: 'var(--sp-6)' }}>
                {Object.entries(STATUS_CONFIG).map(([key, config]) => {
                    const count = projects.filter(p => p.status === key).length;
                    const Icon = config.icon;
                    return (
                        <div key={key} className="card" style={{ padding: 'var(--sp-4)', cursor: 'pointer', 
                            border: statusFilter === key ? '2px solid var(--blue-600)' : '1px solid var(--border-default)' }}
                            onClick={() => setStatusFilter(statusFilter === key ? 'all' : key)}
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginBottom: 'var(--sp-2)' }}>
                                <Icon size={16} style={{ color: config.color }} />
                                <span style={{ fontSize: 'var(--fs-xs)', color: config.color, fontWeight: 'var(--fw-semibold)' }}>
                                    {config.label}
                                </span>
                            </div>
                    ...
                    <table className="table">
                        ...
                        <tbody>
                            {filteredProjects.map((project) => {
                                ...
                                return (
                                    <tr key={project.id}>
                                        <td style={{ fontFamily: 'monospace', fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{project.id}</td>
                                        ...
                                        <td style={{ fontWeight: 'var(--fw-medium)' }}>{formatUSDtoIDR(parseCurrency(project.budget))}</td>
                                        <td>
                                            <span style={{
                                                fontSize: 'var(--fs-xs)',
            ...
            {showModal && selectedProject && (
                <div className="modal-backdrop" onClick={() => setShowModal(false)}>
                    <div className="modal-content" style={{ maxWidth: '700px', maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ borderBottom: '1px solid var(--border-default)', paddingBottom: 'var(--sp-4)', marginBottom: 'var(--sp-4)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--sp-2)' }}>
                                <div>
                                    <span style={{ fontFamily: 'monospace', fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginRight: 'var(--sp-2)' }}>{selectedProject.id}</span>
                                    <span style={{
                                        display: 'inline-flex',
                                        alignItems: 'center',
                                        gap: '4px',
                            ...
                            <h2 style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-bold)' }}>{selectedProject.name}</h2>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-4)', marginBottom: 'var(--sp-4)' }}>
                            <div>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Supplier</label>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginTop: 'var(--sp-1)' }}>
                                    <User size={14} style={{ color: 'var(--text-muted)' }} />
                                    <span>{selectedProject.supplier}</span>
                                </div>
                                <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', marginLeft: '22px' }}>{selectedProject.supplierEmail}</div>
                            </div>
                            <div>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Category</label>
                                <div style={{ marginTop: 'var(--sp-1)' }}>{selectedProject.category}</div>
                            </div>
                            <div>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Budget</label>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginTop: 'var(--sp-1)' }}>
                                    <DollarSign size={14} style={{ color: 'var(--text-muted)' }} />
                                    <span style={{ fontWeight: 'var(--fw-semibold)' }}>{formatUSDtoIDR(parseCurrency(selectedProject.budget))}</span>
                                </div>
                            </div>
                            <div>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Timeline</label>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', marginTop: 'var(--sp-1)' }}>
                        ...
                        <div style={{ marginBottom: 'var(--sp-4)' }}>
                            <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Description</label>
                            <p style={{ marginTop: 'var(--sp-1)', color: 'var(--text-secondary)' }}>{selectedProject.description}</p>
                        </div>

                        <div style={{ marginBottom: 'var(--sp-4)' }}>
                            <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Requirements</label>
                            <p style={{ marginTop: 'var(--sp-1)', color: 'var(--text-secondary)' }}>{selectedProject.requirements}</p>
                        </div>

                        <div style={{ marginBottom: 'var(--sp-4)' }}>
                            <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Deliverables</label>
                            <ul style={{ marginTop: 'var(--sp-1)', paddingLeft: 'var(--sp-4)', color: 'var(--text-secondary)' }}>
                        ...
                        <div style={{ marginBottom: 'var(--sp-4)' }}>
                            <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Tech Stack</label>
                        ...
                        {selectedProject.files.length > 0 && (
                            <div style={{ marginBottom: 'var(--sp-4)' }}>
                                <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Attachments</label>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)', marginTop: 'var(--sp-2)' }}>
                                    {selectedProject.files.map((file, i) => (
                                        <div key={i} style={{
                                            ...
                                            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{file.size}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {/* Notes Section */}
                        <div>
                            <label style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 'var(--fw-semibold)', textTransform: 'uppercase' }}>Activity & Notes</label>
                            <div style={{ marginTop: 'var(--sp-2)', display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
                                {selectedProject.notes.length === 0 ? (
                                    <div style={{ padding: 'var(--sp-4)', textAlign: 'center', color: 'var(--text-muted)', fontSize: 'var(--fs-sm)' }}>
                                        No notes yet
                                    </div>
                                ) : (
                                    selectedProject.notes.map((note, i) => (
                                        <div key={i} style={{
                                            padding: 'var(--sp-3)',
                                            background: note.type === 'clarification' ? 'var(--color-warning-bg)' : 'var(--neutral-50)',
                                            borderRadius: 'var(--radius-md)',
                                            ...
                                            <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>{note.text}</p>
                                        </div>
                                    ))
                                )}
                            </div>
                        ...
                        {/* Action Buttons */}
                        {(selectedProject.status === 'submitted' || selectedProject.status === 'under_review') && (
                            <div style={{ 
                                marginTop: 'var(--sp-6)', 
                                paddingTop: 'var(--sp-4)', 
                                ...
                                <button 
                                    ...
                                    onClick={() => { setShowModal(false); handleAction(selectedProject, 'accept'); }}
                                >
                                    <CheckCircle size={16} /> Accept Project
                                </button>
                                <button 
                                    className="btn btn-ghost"
                                    style={{ flex: 1, color: 'var(--color-danger)', border: '1px solid var(--color-danger)' }}
                                    onClick={() => { setShowModal(false); handleAction(selectedProject, 'reject'); }}
                                >
                                    <XCircle size={16} /> Reject
                                </button>
                                <button 
                                    className="btn btn-ghost"
                                    style={{ flex: 1, color: 'var(--color-warning)', border: '1px solid var(--color-warning)' }}
                                    onClick={() => { setShowModal(false); handleAction(selectedProject, 'clarify'); }}
                                >
                ...
                <div className="modal-backdrop" onClick={() => setActionModal(null)}>
                    <div className="modal-content" style={{ maxWidth: '500px' }} onClick={e => e.stopPropagation()}>
                        ...
                        <p style={{ color: 'var(--text-muted)', marginBottom: 'var(--sp-4)' }}>
                            {actionModal.action === 'accept' && `Are you sure you want to accept "${actionModal.project.name}"?`}
                            {actionModal.action === 'reject' && `Are you sure you want to reject "${actionModal.project.name}"?`}
                            {actionModal.action === 'clarify' && `Send a clarification request to ${actionModal.project.supplier}.`}
                        </p>

                        <div className="form-group">
                            <label className="form-label">
                                {actionModal.action === 'accept' ? 'Note (optional)' : 
                            ...
                            <textarea
                                ...
                                placeholder={
                                    actionModal.action === 'accept' ? 'Add a note about the acceptance...' :
                                    actionModal.action === 'reject' ? 'Explain why the project was rejected...' :
                        ...
                        <div style={{ display: 'flex', gap: 'var(--sp-3)', marginTop: 'var(--sp-4)' }}>
                            ...
                            <button 
                                ...
                                style={{ 
                                    flex: 1,
                                    background: actionModal.action === 'accept' ? 'var(--color-success)' : 
                                               actionModal.action === 'reject' ? 'var(--color-danger)' : 
                                               'var(--color-warning)'
                                }}

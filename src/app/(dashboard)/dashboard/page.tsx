...
import {
    ...
    Plus, ArrowRight, TrendingUp, ChevronRight
} from 'lucide-react';
import { mockProjects, mockSeller, statusLabels, formatDate } from '@/lib/mock-data';
import StatusBadge from '@/components/projects/StatusBadge';
import InsightsCharts from '@/components/dashboard/InsightsCharts';

export default function DashboardPage() {
    const total = mockProjects.length;
    const submitted = mockProjects.filter(p => p.status === 'submitted').length;
    ...
    const needClarification = mockProjects.filter(p => p.status === 'need_clarification').length;
            ...
            <div className="page-header">
                <div>
                    <h1 className="page-title">
                        Good evening, {mockSeller.name.split(' ')[0]} 👋
                    </h1>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
                    ...
                    <div className="card">
                        ...
                        <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)' }}>
                            {[
                                { status: 'submitted', count: submitted, icon: FolderOpen, color: 'var(--blue-500)' },
                                { status: 'under_review', count: underReview, icon: Clock, color: 'var(--color-warning)' },
                                { status: 'need_clarification', count: needClarification, icon: AlertCircle, color: 'var(--color-purple)' },
                                { status: 'accepted', count: accepted, icon: CheckCircle, color: 'var(--color-success)' },
                                { status: 'rejected', count: rejected, icon: XCircle, color: 'var(--color-danger)' },
                        ...
                        <div className="card-body">
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', marginBottom: 'var(--sp-4)' }}>
                                <div style={{
                                    ...
                                    fontWeight: 'var(--fw-bold)', fontSize: 'var(--fs-md)', flexShrink: 0,
                                }}>AT</div>
                                <div>
                                    <div style={{ fontWeight: 'var(--fw-semibold)', fontSize: 'var(--fs-sm)' }}>{mockSeller.company}</div>
                                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{mockSeller.industry}</div>
                                </div>
                            </div>
                            {[
                                { label: 'Contact', value: mockSeller.email },
                                { label: 'Size', value: mockSeller.companySize },
                                { label: 'Member since', value: formatDate(mockSeller.joinedAt) },
                            ].map(({ label, value }) => (
                                <div key={label} style={{
                                    ...
                                    <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', textAlign: 'right', wordBreak: 'break-word' }}>{value}</span>
                                </div>
                            ))}
                        </div>
                    </div>

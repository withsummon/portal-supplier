...
import { mockProjects, formatDate, ProjectStatus } from '@/lib/mock-data';
import StatusBadge from '@/components/projects/StatusBadge';

const statusTabs: { label: string; value: 'all' | ProjectStatus }[] = [
    { label: 'All', value: 'all' },
...
export default function ProjectsPage() {
    ...
    return (
        <div className="animate-in">
            ...
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">
                        {mockProjects.length} project{mockProjects.length !== 1 ? 's' : ''} submitted in total
                    </p>
                </div>
            ...
            <div className="card">
                {/* Tabs + Search */}
                <div style={{ padding: '0 var(--sp-6)', borderBottom: '1px solid var(--border-default)' }}>
                                ...
                                >
                                    ...
                                    {tab.value !== 'all' && (
                                        <span style={{
                                            ...
                                            {mockProjects.filter(p => p.status === tab.value).length}
                                        </span>
                                    )}
                                </button>
                            ))}
                ...
                {filtered.length === 0 ? (
                    <div className="empty-state">
                        ...
                        <h3 style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-semibold)', marginBottom: 'var(--sp-2)' }}>
                            No projects found
                        </h3>
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {filtered.map((project) => (
                                    <tr key={project.id}>
                                        <td style={{ color: 'var(--text-muted)', fontFamily: 'monospace', fontSize: 'var(--fs-xs)' }}>
                                            {project.id}
                                        </td>
                                        <td>
                                        ...
                                        <td style={{ color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>{project.clientName}</td>
                                        <td style={{ color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>{project.category}</td>
                                        <td>
                                            <span style={{
                                                fontSize: 'var(--fs-xs)',
                                                fontWeight: 'var(--fw-semibold)',
                                        ...
                                        <td style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-xs)', whiteSpace: 'nowrap' }}>
                                            {project.budgetRange}
                                        </td>
